...
async function startPluginProcessing(url: string) {
	"use step"
	const run = await start(processPluginWorkflow, [url])
	return run.runId
}

export async function processAllPlugins() {
	"use workflow"
	for (const url of pluginData) {
		await sleep("5s")
		await startPluginProcessing(url)
	}
}

export async function processPluginWorkflow(url: string) {
	"use workflow"
	await doProcessPlugin(url)
}

async function doProcessPlugin(url: string) {
	"use step"
	...
	if (!uPluginFile) {
		...
		const { data: recursiveTreeData } = await octokit.rest.git.getTree({
			owner,
			repo: name,
			tree_sha: data.default_branch,
			recursive: "true",
		...
		uPluginFile = recursiveTreeData.tree.find((file) => {
